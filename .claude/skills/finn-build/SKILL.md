...
Using the Linear connector, list issues on team `AIL` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `AIL-NNN-short-slug`, using the issue's real identifier.
...
- `Closes AIL-NNN`, using the real Linear issue identifier
