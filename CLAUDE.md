...
| `stonescan/ingest.py` | Orchestrator: merge any spill → crawl → normalize → store → history snapshot |
| `stonescan/spill.py` | Folds a spill crawl (AIL-28's fallback catalog on C:) back into the primary. Per supplier, **strictly newest `crawled_at` wins** — which is also why a second merge is a no-op, since merging copies the stamps. Replays rows through the crawler's own `replace_materials`/`replace_slabs` (ids are reassigned every crawl), in **one** transaction, after a backup. Only crawl-owned tables move; a schema-mismatched spill is refused rather than merged |
